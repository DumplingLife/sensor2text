...
class Model(nn.Module):
    def __init__(self, dims, nhead, num_layers, output_size, dropout, use_cls_token=True, use_pos_encoder=True):
        super().__init__()
        self.cls_token = nn.Parameter(torch.randn(1, 1, dims)) if use_cls_token else None
        self.pos_encoder = PositionalEncoding(dims, dropout) if use_pos_encoder else None
        self.encoder = nn.TransformerEncoder(nn.TransformerEncoderLayer(dims, nhead, dropout=dropout), num_layers=num_layers)
        self.output_projection = nn.Linear(dims, output_size)
...
class AllSensorsModel(nn.Module):
    def __init__(self):
        ...
        self.encoders = nn.ModuleDict({
            modality: Model(dims=input_size, nhead=2, num_layers=4, dropout=0.1,
